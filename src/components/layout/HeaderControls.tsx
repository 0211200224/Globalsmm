...
/**
 * Theme + language controls, meant to live in the header on every screen —
 * including pre-login (login/register), which is why this doesn't assume an
 * authenticated shell. Locale is a cookie (see setLocale/get-dictionary.ts)
 * so it round-trips correctly even before signing in; theme is
 * localStorage-only (see src/lib/theme.ts), same as the settings page.
 */
